...
std::ostream& operator<<( std::ostream &out, Square &cRec ) {
	out << "x:[" << cRec.X_start << ", " <<  cRec.X_end << "]  " << "y:[" << cRec.Y_start << ", " <<  cRec.Y_end << "]  " <<
			"-index: [" << cRec.recIndex_horizontal << ", " <<  cRec.recIndex_vertical << "]   - nEdge: " << cRec.nEdgeToGenerate <<" precentage:"<<  (double)cRec.nEdgeToGenerate/((cRec.X_end - cRec.X_start)*(cRec.Y_end - cRec.Y_start)) <<std::endl;
	return out;
}
